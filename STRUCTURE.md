...
│   │       ├── faq.py        # Truy hồi FAQ
│   │       ├── data.py       # Câu cần dữ liệu sống (gọi BE)
...
| [`routes.py`](app/api/routes.py) | Định nghĩa `POST /api/chat`; khai báo Pydantic schema `ChatRequest`/`ChatResponse`; chuyển request → `ChatContext`, gọi engine, trả kết quả. |
...
| [`backend.py`](app/clients/backend.py) | Client httpx gọi **ngược** về Backend: `get_subjects()`, `search_tutors()`. Bóc `data` trong response `{success,message,data}`; nuốt mọi lỗi mạng → trả `None` (để resolver rơi xuống AI, không làm sập chat). |
...
| [`types.py`](app/core/types.py) | Các dataclass thuần: `Source` (enum nguồn), `Message`, `ChatContext`, `Resolution` (câu trả lời + confidence + source + meta). |
...
| [`registry.py`](app/core/resolvers/registry.py) | `build_resolvers()` — nạp `intents.json`/`faq.json`, lắp chuỗi theo thứ tự `greeting → data → keyword → faq`; trả kèm `FaqResolver` để engine dùng cho RAG. |
...
| [`conftest.py`](tests/conftest.py) | Fixtures dùng chung: `FakeProvider` (AI giả, không gọi mạng), `FakeBackend`, `engine`, `client` (FastAPI TestClient). |
| [`test_resolvers.py`](tests/test_resolvers.py) | Test `normalizer` + từng resolver (greeting/keyword/faq/data). |
...
routes.py  ──►  deps.get_engine()  ──►  core/engine.py: ChatEngine.answer()
        ...
        ┌─────────────────────────────────────┼───────────────────────────────┐
        ▼                                       ▼                               ▼
   cache.py (hit?)          resolvers/* (greeting→data→keyword→faq)      providers/groq.py
        │                    │ dùng normalizer.py + base.py helper        │ (khi rule bó tay,
        │                    │ data.py → clients/backend.py               │  kèm prompt.py RAG)
        └───────────────────►└────────────────────────────────────────────┘
                                              │
                                              ▼
